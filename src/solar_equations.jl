...
function apply_solar!(
    ...
    if use_reg && use_solar_reg
        reg⁺_S = JuMP.@variable(
            jump_model,
            0 <=
            reg⁺_S[j in scenarios, t in time_steps] <=
    ...
    if use_spin && use_solar_spin
        spin_S = JuMP.@variable(
            jump_model,
            0 <=
            spin_S[j in scenarios, t in time_steps] <=
    ...
    if use_reg && (use_solar_reg || use_solar_spin)
        if use_solar_reg
            solar_reserve_dn_constraint = JuMP.@constraint(
                jump_model,
                [j in scenarios, t in time_steps],
                reg⁻_S[j, t] <= pS[j, t]
            )
        end
        solar_reserve_up_constraints = JuMP.@constraint(
            jump_model,
            [j in scenarios, t in time_steps],
            (use_solar_reg ? reg⁺_S[j, t] : 0) + (use_solar_spin ? spin_S[j, t] : 0) <=
...
function apply_solar!(
    ...
    # Solar power
    solar_constraints =
        JuMP.@constraint(jump_model, [t in time_steps], pS[t] <= total_solar[t])

    # Solar reserve holding
    if use_reg && use_solar_reg
        solar_reserve_dn_constraint =
            JuMP.@constraint(jump_model, [t in time_steps], reg⁻_S[t] <= pS[t])
    end
    if (use_reg || use_solar_reg) && (use_spin || use_solar_spin)
        solar_reserve_up_constraints = JuMP.@constraint(
            ...
            (use_spin && use_solar_spin ? spin_S[t] : 0) <= total_solar[t] - pS[t]
        )
    end
