...
function apply_storage!(
    ...
    pb_in_max = Dict(
        b =>
            ...
            problem.ext["storage_scale"] for b in storage_names
    )
    pb_out_max = Dict(
        b =>
    ...
    optimization_container.variables[:E__GenericBattery] =
        eb = JuMP.@variable(
            jump_model,
    ...
    storage_energy_balance =
        JuMP.Containers.DenseAxisArray{JuMP.ConstraintRef}(undef, storage_names, time_steps)
