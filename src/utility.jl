...
function get_thermal_generator_power_dataframe(
    ...
    if length(size(pg)) == 2
        pg_dataframe = PSI.axis_array_to_dataframe(jump_model.obj_dict[:pg], [:pg])[time_steps, :]
    elseif length(size(pg)) == 3
        pg_dataframe = _scenario_in_3D_array_to_dataframe(
            pg,
            scenario,
            time_steps)
    else
...
function write_missing_power(
    ...
    total_solar_GW = h5open(joinpath(data_path, "ERCOT132.h5"), "r") do file
        return read(file, "Power")[(288*(d-1)+1):(288*(d-1) + length(time_steps)*12)] ./ 1000
    end

    if length(size(jump_model.obj_dict[:pg])) == 2
        total_thermal = sum.(eachrow(get_thermal_generator_power_dataframe(
            problem,
            time_steps,
            nothing)))
    else
    ...
    pb_in = sum.(eachrow(PSI.axis_array_to_dataframe(jump_model.obj_dict[:pb_in], [:pb_in])))
    pb_out = sum.(eachrow(PSI.axis_array_to_dataframe(jump_model.obj_dict[:pb_out], [:pb_out])))
...
_missing_power_by_5_min = function(
    ...
    for i in time_steps
        for l in get_components(PowerLoad, system_ha)
            total_load[((i - 1) * time_steps_per_hour + 1):(i * time_steps_per_hour)] .+= get_time_series_values(
                Deterministic,
                l,
                "max_active_power";
    ...
    for i in time_steps
        for l in get_components(HydroGen, system_ha)
            total_hydro[((i - 1) * time_steps_per_hour + 1):(i * time_steps_per_hour)] .+= get_time_series_values(
                Deterministic,
                l,
                "max_active_power";
                start_time = case_initial_time + Hour(i - 1),
    ...
    for i in time_steps
        for l in get_components(RenewableGen, system_ha, x -> get_prime_mover(x) != PrimeMovers.PVe)
            total_wind[((i - 1) * time_steps_per_hour + 1):(i * time_steps_per_hour)] .+= get_time_series_values(
                Deterministic,
                l,
                "max_active_power";
                start_time = case_initial_time + Hour(i - 1),
    ...
    missing_power = (total_load - total_hydro - total_wind +
        repeat(pb_in - pb_out - total_thermal, inner = 12)
    ) .* 100 ./ 1000 - total_solar_GW
    missing_power = DataFrame(transpose(reshape(missing_power, (12, length(time_steps)))), :auto)
    CSV.write(joinpath(output_path, "missing_power__5min_setpoint.csv"), missing_power)

    missing_capacity = (total_load - total_hydro - total_wind  + 
    ...
    CSV.write(joinpath(output_path, "missing_capacity__5min_setpoint.csv"), missing_capacity)
...
_missing_power_by_hourly_setpoint = function(
    ...
    net_GW = (total_load + pb_in - pb_out - total_thermal - total_hydro - pW[:, 1]) .* 100 ./ 1000
    ...
    net_GW_online = (total_load + pb_in - pb_out - total_thermal_online - total_hydro - total_wind) .* 100 ./ 1000
