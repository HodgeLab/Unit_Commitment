...
function apply_wind!(
    ...
    if use_reg && use_wind_reserves
        reg⁺_W = JuMP.@variable(
            jump_model,
            0 <= reg⁺_W[t in time_steps] <= required_reg⁺[t] .* problem.ext["wind_reg_prop"]
        )
        reg⁻_W = JuMP.@variable(
            jump_model,
            0 <= reg⁻_W[t in time_steps] <= required_reg⁻[t] .* problem.ext["wind_reg_prop"]
    ...
    if use_spin && use_wind_reserves
        spin_W = JuMP.@variable(
            jump_model,
            0 <= spin_W[t in time_steps] <= required_spin[t] .* problem.ext["wind_spin_prop"]
        )
    end
    ...
    # Wind constraint
    wind_constraint = JuMP.@constraint(
        jump_model,
        [t in time_steps],
        pW[t] <= total_wind[t]
    )
